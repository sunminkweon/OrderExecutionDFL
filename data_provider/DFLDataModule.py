...
class DFLDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if self.directory.find('simulation') != -1 :
        ...
        train_X = train_X.reshape(-1,self.time_length, self.time_length)
        val_X = val_X.reshape(-1,self.time_length, self.time_length)
        test_X = test_X.reshape(-1,self.time_length, self.time_length)
        
        train_Y = train_Y.reshape(-1,self.time_length, self.time_length)
        val_Y = val_Y.reshape(-1,self.time_length, self.time_length)
        test_Y = test_Y.reshape(-1,self.time_length, self.time_length)
...
def initialize_data_module(args, step):
    ...
    data_module = DFLDataModule(args, step)
